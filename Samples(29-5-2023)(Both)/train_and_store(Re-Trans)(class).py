...
def classifyPipeline(X, Y):
    pipe = pipeline.Pipeline([('sc', preprocessing.StandardScaler()), ('pca', decomposition.PCA(n_components=4)), ('svc', svm.SVC(gamma='auto', kernel='rbf', probability=False))], verbose=True)
    pipe.fit(X, Y)
    return pipe
...
joblib.dump((Both_empty_or_not_pipe, Both_classify_pipe), 'trained_pipes(29-5-2023)(Both)v7.joblib')